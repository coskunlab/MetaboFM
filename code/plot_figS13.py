...
plot_figS13.py
--------------
Supplementary Figure S13: Drug Landscape — Extended Analysis.
...
Usage:
  conda run -n torch_gpu python plot_figS13.py
"""
...
PANEL_DIR = OUT_DIR / "figS13_drug_extended"
...
CAPTION = """\
Supplementary Figure 13 | Extended drug proximity analysis.
...
def main():
    ...
    draw_panel_a(ax, df)
    save_panel(fig, "figS13_panelA_drug_similarity_dist")
    plt.close(fig)

    # Panel B
    fig, ax = plt.subplots(figsize=(7.0, 6.5))
    draw_panel_b(ax, df)
    save_panel(fig, "figS13_panelB_top20_nondrug")
    plt.close(fig)

    # Panel C
    fig, ax = plt.subplots(figsize=(7.0, 5.5))
    draw_panel_c(ax, df, fig)
    save_panel(fig, "figS13_panelC_pca_drug_similarity")
    plt.close(fig)
